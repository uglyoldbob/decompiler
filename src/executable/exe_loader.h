...
class exe_loader
{
	public:
		...
		virtual ~exe_loader();
		virtual int process(std::istream *me) = 0; ///< Perform basic processing of the executable file. Used to do things such as interpret executable header information.
		...
		static exe_loader* check(std::istream *me); ///< Call all registered exe_checker functions to determine which exe_loader to use for the specified input stream (of executable binary contents)
	protected:
		std::istream *exe; ///< The input stream for reading the executable
